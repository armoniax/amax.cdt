...
In `eosio.cdt` v1.6.0, `eosiolib` will now be partitioned into 3 groups. These allow for finer grained allowance for particular modes of compilation.
  ...
  - Contracts
  - Core
  - The `CAPI` will only be available to contract writers using `amax-cc` and purely for `C` smart contracts.
  - The `Contracts` will be available to `amax-cpp` for smart contract writing. 
  - `Core` will be available to `amax-cpp` for any of the modes (present and future).  
  - `amax-cpp` in `fnative` mode will have access to all of these groups.
...
This entire API is now only available to C developers (i.e. using `amax-cc`), and all internal uses in `Core` and `Contracts` have been guarded behind the namespace `eosio::internal_use_do_not_use`.  
