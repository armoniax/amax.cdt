...
As of v1.5.0 native compilation can be performed and a new set of libraries to facilitate native testing and native "scratch pad" compilation. [`amax-cc`](../03_command-reference/amax-cc.md), [`amax-cpp`](../03_command-reference/amax-cpp.md) and [`amax-ld`](../03_command-reference/amax-ld.md) now support building "smart contracts" and unit tests natively for quick tests to help facilitate faster development \(note the default implementations of eosio `intrinsics` are currently asserts that state they are unavailable, these are user definable.\)
